...
void main() {
  ...
  group('ChatSessionCubit', () {
    ...
    test(
      'history replace keeps newer assistant content for the same assistant id',
      () async {
        final cubit = createCubit('s1', provider: Provider.codex);
        addTearDown(cubit.close);
        mockBridge.emitMessage(
        ...
        mockBridge.emitMessage(
          AssistantServerMessage(
            message: AssistantMessage(
              ...
              role: 'assistant',
              content: [const TextContent(text: 'short draft')],
              model: 'codex',
            ),
          ),
          sessionId: 's1',
        ...
        mockBridge.emitMessage(
          HistoryMessage(
            messages: [
              AssistantServerMessage(
                message: AssistantMessage(
                  ...
                  content: [
                    const TextContent(
                      text:
                          'short draft\n\nLatest complete answer that should stay visible.',
                    ),
                  ],
                  model: 'codex',
                ),
              ...
              const ResultMessage(
                subtype: 'success',
                result: 'Latest complete answer that should stay visible.',
              ),
            ],
          ),
          sessionId: 's1',
        ...
        expect(
          textParts,
          contains('Latest complete answer that should stay visible.'),
        );
      },
    );
    ...
    test(
      ...
      () async {
        ...
        mockBridge.emitMessage(
          AssistantServerMessage(
            message: AssistantMessage(
              id: 'assistant-downgrade-check',
              role: 'assistant',
              content: [
                const TextContent(
                  text: 'Final answer that must remain visible.',
                ),
              ],
              model: 'codex',
            ),
                ...
                message: AssistantMessage(
                  ...
                  content: [
                    const ThinkingContent(
                      thinking: 'Finalizing report details',
                    ),
                  ],
                  model: 'codex',
                ),
        ...
        final textParts = assistantMessage.message.content
            ...
            .map((content) => content.text)
            .join('\n');
        expect(textParts, contains('Final answer that must remain visible.'));
        expect(
